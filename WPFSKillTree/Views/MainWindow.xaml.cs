...
/*#if (PoESkillTree_DisableStatTracking == false)
        ...
        /// <summary>
        /// Gets all values of the OffHand Enum.
        /// </summary>
        public static IEnumerable<OffHand> OffhandValues => Enum.GetValues(typeof(OffHand)).Cast<OffHand>();

        /// <summary>
        /// Primary weapon used for pseudo attribute calculations.
        /// </summary>
...
/*#if PoESkillTree_DisableStatTracking==false
        ...
        /// <summary>
        /// Clicking the Skill Tree background.
        /// </summary>
                    ...
                    else
                    {
                        ...
                        if (Tree.SkilledNodes.Contains(node))
                        {
                            if(node.PassiveNodeType == PassiveNodeType.Mastery && Keyboard.Modifiers.HasFlag(ModifierKeys.Alt))//Undo Mastery Option Via Alt+Click
                            {
                                ...
                                SetCurrentBuildUrlFromTree();
                                UpdateUI();
                                return;//Allow to unselect mastery choice without unskilling the node
                            }
                            else if(node.IsAscendancyStart)//Clicking on Ascendancy Start node functions to remove all ascendancy points
                            {
                        ...
                        else if (_prePath != null)
                        {
                            ...
                            {
                                Tree.AllocateAscSkillNodes(_prePath);
                            }
                            else
                            {
